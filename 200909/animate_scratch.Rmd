...
p <- fr %>% 
  ...
  geom_image(aes(x=cumsum,image = img,y=rank-.3),size=.14)+
  ...
  geom_text_repel(aes(x=cumsum,label=paste(cumsum, "      ", sep=""),color=speaker),vjust=.5,hjust = 1,fontface='bold',size = 9,nudge_x=-20,nudge_y=.22,force=0,segment.alpha=0)+
  geom_text_repel(aes(x=cumsum,label=paste("+",step, "              ",sep=""),color=speaker),vjust=.5,hjust = 1,fontface='bold.italic',nudge_x=-20,nudge_y=-.1,force=0,segment.alpha=0)+
  scale_y_reverse()+
  ...
  theme(
    ...
    plot.margin=unit(c(3.5,7,3.5,7),"cm"),
    ...
    axis.title.x = element_text(size = 8, color="#ffffff"),
